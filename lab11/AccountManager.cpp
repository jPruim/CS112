...
void AccountManager::changeUserName(const string& oldUserName, const string& newUserName) {
	...
	if(it == myAccounts.end()){
		throw Exception ("change user name", "Name not in map");
	}
	string pwd = it->second;
	deleteAccount(oldUserName);
	createNewAccount(newUserName, pwd);
}
...
void AccountManager::deleteAccount(const string& userName) {
	map<string,string>::iterator iter = myAccounts.find(userName);
	if (iter == myAccounts.end()){
		throw Exception("deleteAccount", "not an Account");
	}
	myAccounts.erase(iter);
}
...
void AccountManager::changePassword(const string& userName,
		...
		if(it == myAccounts.end()){
			throw Exception("change password", "not an account" );
		}
